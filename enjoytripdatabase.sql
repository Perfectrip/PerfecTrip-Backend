...
CREATE TABLE IF NOT EXISTS `enjoytrip`.`user` (
  ...
  `password` VARCHAR(10000) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `enjoytrip`.`plan_board` (
  ...
  `plan_order` VARCHAR(2000) NULL DEFAULT NULL,
  `article_content` VARCHAR(2000) NULL DEFAULT NULL,
  `view_cnt` INT NULL DEFAULT NULL,
  `register_time` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  INDEX `plan_board_to_user_id_fk_idx` (`user_id` ASC) VISIBLE,
  CONSTRAINT `plan_board_to_user_id_fk`
    FOREIGN KEY (`user_id`)
